/*Ovaj program učitava podatke iz datoteke ispit.txt u niz struktura studenti, zatim sortira niz
 struktura prema broju bodova od većeg prema manjem, te ispisuje sadržaj sortiranog niza
 studenti u datoteku ispit_sortiran.txt*/
#include <stdio.h>
...
int main()
{
 ...
 return 1;
    ...
    while (fscanf(ulaz, "%20s%15s%2d%\n", studenti[i].prezime,
                  studenti[i].ime, &studenti[i].broj_bodova) == 3 &&
           i < 300)
        i++;
    vel = i;
    ...
    for (i = 0; i < vel; i++)
    {
        max = i;
        for (j = i + 1; j < vel; j++)
            if (studenti[j].broj_bodova >
                studenti[j].broj_bodova)
                max = j;
        tmp = studenti[i];
    ...
    for (i = 0; i < vel; i++)
        fprintf(izlaz, "%-20s%-15s%2d\n", studenti[i].prezime,
                studenti[i].ime, studenti[i].broj_bodova);
